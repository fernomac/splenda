...
func (d *DB) NewUser(id string, hash string) error {
	db, err := d.open()
	if err != nil {
		return err
	}
	...
	_, err = db.Exec("INSERT INTO users (id, hash) VALUES ($1, $2)", id, hash)
	if err != nil {
...
func (d *DB) GetUserHash(id string) (string, error) {
	db, err := d.open()
	if err != nil {
		return "", err
	}
	...
	hash := ""
	row := db.QueryRow("SELECT hash FROM users WHERE id=$1", id)
...
// RemoveCard removes a card from the board when the corresponding deck is empty.
func (t *TX) RemoveCard(tier int, index int) error {
